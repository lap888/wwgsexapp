                    ...
                    {data && data.length > 0 && data.map((item, index) => {
                        return (
                            <TouchableOpacity activeOpacity={0.8} key={index} style={styles.item} onPress={() => this.setState({selected: item})}>
                                <Text style={{fontSize: 12 }}>{item.shares}张认证劵</Text>
                                <Text style={{fontSize: 15, fontWeight: 'bold', marginTop: 3 }} >{item.cash}<Text style={{fontSize: 13, fontWeight: 'normal'}}>元</Text></Text>
