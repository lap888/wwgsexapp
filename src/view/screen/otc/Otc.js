...
class Otc extends Component {
  ...
  onPressSell(item) {
    ...
    if (!this.props.isIgnored) {
      Alert.alert(
        "交易规则",
        `1.支付倒计时时间为60分钟 发送倒计时时间为60分钟
2.如果卖家支付宝搜索不到或无法验证付款等 直接提供打款异常截图即可
`,
        [
          {
            text: "不再提示", onPress: () => {
  ...
  renderHeaderComponentNew() {
    return (
      <View style={{ height: 40, width: Metrics.screenWidth, backgroundColor: Colors.C9, alignItems: 'center', justifyContent: 'center', }}>
  ...
  render() {
    return (
      <View style={{ flex: 1, backgroundColor: Colors.White }}>
        <Header title={'交易'} leftText='规则' onLeftPress={() => { this.onLeftPress() }} leftStyle={{ color: Colors.titleMainTxt }} titleStyle={{ color: Colors.titleMainTxt, fontWeight: 'bold' }} backgroundColor={Colors.White} rightText="订单" rightStyle={{ color: Colors.titleMainTxt }} onRightPress={() => this.onRightPress()} />
