...
export default class MiningMachinery extends Component {
        ...
        if (data.status === 1) {
            if (data.minningStatus === 0) {
                return (
                    <TouchableOpacity style={styles.btnView} onPress={() => this.doTask(data.id)}>
                        <Text style={styles.btnTxt}>开始挖矿</Text>
                    </TouchableOpacity>
                )
            ...
            return (
                <TouchableOpacity style={styles.btnView} onPress={() => this.props.repair(data.id)}>
                    <Text style={styles.btnTxt}>修复矿机</Text>
                </TouchableOpacity>
            )
        }else {
            <View style={styles.btnView}>
    ...
    render() {
        ...
        return (
            <View style={styles.itemView}>
                {data.minningId != 0 && <View style={{paddingHorizontal: 10, justifyContent: 'center'}}>
                    <Text style={{fontSize: 10, color: Colors.greyText}}>时效: {String(data.beginTime).split(' ')[0]} ~ {String(data.endTime).split(' ')[0]}</Text>
                </View>}
                ...
                <View style={{justifyContent: 'center', alignItems: 'center', marginTop: 10}}>
                    <View style={{width: 170, height: 40, borderRadius: 20, backgroundColor: data.colors}}>
                        {this.btnContent()}
                    </View>
                </View>
                <View style={{marginLeft: 25, marginTop: 15}}>
                ...
                <View style={{flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 10, marginTop: 5}}>
                    <Text style={{fontSize: 10, color: Colors.greyText}}>挖矿时间: 6:00 ~ 21:00</Text>
                    <Text style={{fontSize: 10, color: Colors.greyText}}>收矿时间: 9:00 ~ 24:00</Text>
                </View>
                <View style={[styles.tag, {borderLeftColor: data.colors, borderRightColor: data.colors, borderTopColor: data.colors}]}/>
            </View>
        );
    }
}
